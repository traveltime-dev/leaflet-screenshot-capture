...
const timeMapResponseToGeoJSON = (response, bounds) => {
  const featureList = response.results.map((result) => {
    ...
    return {
      ...
      id: result.search_id,
      properties: {
        opacity: config.colors.borderOpacity || 0,
        fill: true,
        fillColor: config.colors.fillColor || '#38174f',
        color: config.colors.borderColor || '#38174f',
        fillOpacity: config.colors.fillOpacity || 0.5,
      },
      geometry: {
        type: 'Polygon',
        coordinates,
